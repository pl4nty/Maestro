...
namespace Maestro
{
    public class ExecIntuneUploadCommand
    {
        public static async Task Execute(CommandLineOptions options, LiteDBHandler database)
        {
            ...
            // Validate SAS URL
            if (!Uri.IsWellFormedUriString(options.SasUrl, UriKind.Absolute))
            {
                Logger.Error("Invalid SAS URL.");
                return;
            }
            ...
            if (options.Events != null)
            {
                foreach (var eventLog in options.Events)
                {
                    ...
                    {
                        Logger.Error("Event log channel cannot be empty.");
                        return;
                    }
                }
            }
            ...
            string policyId = await intuneClient.CreateCustomConfigPolicy(options.Device, options.SasUrl, options.RegistryKeys, options.Events, options.Commands, options.FolderFiles, options.OutputFileFormat);
            if (policyId is null) return;

            // Assign the policy with the filter
            bool success = await intuneClient.AssignPolicyWithFilter(policyId, filterId);
            if (!success) return;

            Logger.Info("Successfully created and assigned custom config policy.");
        }
    }
}
